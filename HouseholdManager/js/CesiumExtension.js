...
var CustomPolygonEntity=(function () {
    function _(viewer,positions,options) {
        ...
        Cesium.knockout.track(this, [ 'positions']);
    }
...
var DrawHelper=(function () {
    ...
    function rightClickEvent(drawHelper,ev) {
        var that=drawHelper;
        var pos=that._getMousePos(ev.position);
        if(!Cesium.defined(pos)){
            return;
        ...
        if(that._drawable){
            ...
            if(Cesium.defined(that._oRightClickEvent)){
                that._oRightClickEvent(ev);
            }
        }
    }
        ...
        if(that._drawable){
            ...
            switch (len) {
                ...
                case 1:
                    ...
                    break;
                default:
                    that._toolTips.showAt('单击添加面的节点，右键结束绘制',ev.endPosition);
    ...
    function _(viewer,drawFinishedCallback,selectChangedCallback) {
        ...
        Cesium.knockout.track(this, [ '_currentCustomPolygon']);
    }
    ...
    _.prototype._initHandler=function () {
        ...
        this._oRightClickEvent=handler.getInputAction(Cesium.ScreenSpaceEventType.RIGHT_CLICK);
